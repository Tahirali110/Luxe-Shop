...
const HeroOrbit = () => {
  const [orbitProducts, setOrbitProducts] = useState<Product[]>([]);
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        setOrbitProducts(response.data.slice(0, 3));
      } catch (error) {
  ...
  if (isLoading || orbitProducts.length === 0) {
    return (
      <div className="relative w-80 h-80 md:w-[420px] md:h-[420px] lg:w-[500px] lg:h-[500px] flex items-center justify-center">
        <Skeleton className="w-48 h-48 rounded-full" />
      </div>
    );
  }

  const mainProduct = orbitProducts[0];
  const mainImage = mainProduct.colors?.[0]?.image || mainProduct.image || '';
  ...
  return (
    <div className="relative w-80 h-80 md:w-[420px] md:h-[420px] lg:w-[500px] lg:h-[500px]">
      ...
      {orbitProducts.map((product, index) => {
        ...
        const productImage = product.colors?.[0]?.image || product.image || '';
