...
const CheckoutContent = () => {
  ...
  const handlePlaceOrder = async () => {
    ...
    const orderId = `LXR-${Date.now().toString().slice(-8)}`;
    setGeneratedOrderId(orderId);

    const selectedShipping = SHIPPING_METHODS.find(m => m.id === formData.shippingMethod) || SHIPPING_METHODS[0];
    const now = new Date();
    ...
    const orderData: Order = {
      ...
      estimatedDelivery: selectedShipping.duration,
      paymentMethod: formData.paymentMethod === 'credit-card' ? 'Credit Card' :
        formData.paymentMethod === 'upi' ? 'UPI' :
          formData.paymentMethod === 'wallet-apps' ? (SUPPORTED_PAYMENT_APPS.find(a => a.id === formData.selectedApp)?.name || 'Payment App') : 'Other',
      status: 'placed'
    };

    lastOrderData.current = orderData;
    console.log('Order data captured:', orderData);

    // Save to global order store
    useOrderStore.getState().addOrder(orderData);
