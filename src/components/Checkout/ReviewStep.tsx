...
export const ReviewStep = ({ onBack, onPlaceOrder, isProcessing, onEditStep }: ReviewStepProps) => {
  ...
  useEffect(() => {
    // #region agent log
    fetch('http://127.0.0.1:7242/ingest/3e34ee89-e415-44aa-ad50-a9a60ca8d5a2', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ location: 'src/components/Checkout/ReviewStep.tsx:ReviewStep', message: 'Review pricing snapshot', data: { subtotal, discount, shipping: shippingCost, tax, total, promoCode }, timestamp: Date.now(), sessionId: 'debug-session', runId: 'run1', hypothesisId: 'C' }) }).catch(() => { });
    fetch('http://127.0.0.1:7242/ingest/3e34ee89-e415-44aa-ad50-a9a60ca8d5a2', { method: 'POST', mode: 'no-cors', headers: { 'Content-Type': 'text/plain' }, body: JSON.stringify({ location: 'src/components/Checkout/ReviewStep.tsx:ReviewStep', message: 'Review pricing beacon', data: { subtotal, discount, shipping: shippingCost, tax, total, promoCode }, timestamp: Date.now(), sessionId: 'debug-session', runId: 'run1', hypothesisId: 'C' }) }).catch(() => { });
    // #endregion
  }, [subtotal, discount, shippingCost, tax, total, promoCode]);
