...
// Mock orders with mathematically correct totals
const mockOrdersData = [
  {
    ...
    shippingFee: 10,
    items: [
      { ...products[0], id: String(products[0].id), quantity: 1, selectedColor: 'Black', selectedSize: 'M', image: products[0].colors[0].image },
      { ...products[1], id: String(products[1].id), quantity: 1, selectedColor: 'Navy', selectedSize: 'L', image: products[1].colors[0].image },
    ],
    shippingAddress: {
      label: 'Home',
      firstName: 'John',
  ...
  },
  {
    ...
    shippingFee: 15,
    items: [
      { ...products[2], id: String(products[2].id), quantity: 2, selectedColor: 'White', selectedSize: 'S', image: products[2].colors[0].image },
    ],
    shippingAddress: {
      label: 'Office',
      firstName: 'John',
  ...
  },
  {
    ...
    shippingFee: 0,
    items: [
      { ...products[3], id: String(products[3].id), quantity: 1, selectedColor: 'Gray', selectedSize: 'XL', image: products[3].colors[0].image },
    ],
    shippingAddress: {
      label: 'Home',
      firstName: 'John',
...
export const OrdersSection = () => {
  ...
  // Combine mock orders with real store orders
  const allOrders = useMemo(() => [...storeOrders, ...mockOrders], [storeOrders]);
  ...
  const handleReorder = (orderId: string) => {
    try {
      ...
      order.items.forEach(item => {
        const baseProduct = products.find(p => String(p.id) === String(item.id));
        if (baseProduct) {
          const colorVariant = baseProduct.colors.find(c => c.name === item.selectedColor);
          // Add precisely the requested quantity
    ...
    printWindow.document.write(`
      <html>
        ...
        <body>
          ...
          <div class="footer">Thank you for shopping with LUXE!</div>
          <script>
            window.onload = () => {
              window.print();
                  ...
                  <div className="relative mb-4">
                    <div className="flex justify-between items-center">
                      {orderStatusSteps.map((step, index) => {
                        ...
                        const timeLabel = (step === 'placed' && order.placedAt) ? order.placedAt :
                          (step === 'processing' && order.paymentConfirmedAt) ? order.paymentConfirmedAt : null;
                        ...
                        return (
                          <div key={step} className="flex flex-col items-center relative z-10">
                            ...
                            <div className="flex flex-col items-center mt-2">
                              ...
                              {isActive && timeLabel && (
                                <span className="text-[8px] text-primary font-bold whitespace-nowrap">
                                  {timeLabel}
                                </span>
                              )}
                            </div>
                      ...
                      <div className="p-4 sm:p-6 bg-secondary/30 space-y-6">
                        ...
                        <div>
                          ...
                          <div className="space-y-3">
                            {order.items?.map((item, idx) => (
                              <Link key={idx} to={`/product/${item.id}`} className="block">
                                <motion.div
                                  whileHover={{ x: 4 }}
                        ...
                        <div className="grid sm:grid-cols-2 gap-6">
                          ...
                          <div>
                            ...
                            <div className="p-3 bg-card rounded-xl space-y-2 text-sm">
                              ...
                              <div className="flex justify-between">
                                <span className="text-muted-foreground">Shipping ({order.shippingMethodName || 'Standard'})</span>
